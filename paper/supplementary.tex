...
Because all closed-form laws are expectations over the random-key ensemble while every trained experiment shares a single fixed codebook (Supp.~F), an additional synthetic probe (\texttt{tests/codebook\_ensemble\_probe.py}) repeats the algebraic $2\times2\times\delta$ computation over eight independent codebooks at $D=64$, $n=20$. Inter-codebook dispersion is small: the largest standard deviation of $L$ across codebooks is $0.012$ ($\delta=0.5$, state/complex), and per-codebook maximum absolute deviations range from $0.004$ to $0.024$. The fixed codebook used throughout the trained experiments lies within this ensemble spread.
